...
public class Arrow : MonoBehaviour {
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        {
            //Debug.Log("hit");
            DoBulletHit(other.gameObject);
        }
    }

    void DoBulletHit(GameObject input)
    {
        //if (radius == 0)
        //{
            input.GetComponent<Griefer>().TakeDamage(damage, type);
        /*}
        else
        {
    ...
    public void Set(Transform inTarget, float inDamage, int inType)
    {
        target = inTarget;
        damage = inDamage;
        type = inType;
